...
// 내 생각엔 모든 경로 탐색할 수 있는 정확한 알고리즘인 것 같은데 재귀함수 때문인지 N=8만 되어도 StackOverFlow 발생
// 이 코드도 가치가 있으니 남겨두고(reserveA) 반복문 혹은 더 최적화된 방식 있는지 고민..

public class D4_1249_reserveA {
	public static int N;
	public static int[][] map;
	public static List<Deque<Pos_reserveA>> cases;
		...
		try (Scanner sc = new Scanner(System.in)) {
			...
			for (int t=1; t<=T; t++) {
				...
				/* 수정 : 굳이 2차원으로 할 필요가 없는 것 같다
						 Deque<Pos_reserveA>로 처리하고 'pop -> 처리' 반복해도 될듯
				...
				*/
				...
				/*
				 ...
				 2중 deque보다는 List<Deque<Pos_reserveA>>가 나을 듯. 각 Step은 번호로 접근하는 게 더 편함
				*/
				
				cases = new ArrayList<>();
				cases.add(new ArrayDeque<Pos_reserveA>());
				cases.get(0).add(new Pos_reserveA(0, 0));
	...
	static void searchPath(int cost) {
		...
		if (cases.get(cases.size()-1).size() == 0) { // 마지막 Step에 후보가 더 없으므로 해당 Step은 지우고 그 직전의 Head도 pop
			...
			if (0 < cases.size()) { // S 말고 더 남았으면 이미 탐색한 후보 1개 지우고 다시 재귀. S뿐이면 이대로 재귀 종료
				Pos_reserveA head = cases.get(cases.size()-1).pop();
			...
			Pos_reserveA head = cases.get(cases.size()-1).peekFirst(); // 후보들 중 선택된 좌표
			if (head.r == N-1 && head.c == N-1) { // 선택된 좌표가 곧 종점(G)일 경우, minTotal 갱신 후 재귀
				...
				Deque<Pos_reserveA> newStep = new ArrayDeque<>();
				
				Pos_reserveA up = newCandidate(head.r - 1, head.c, cost);
				if (up != null) newStep.add(up);

				Pos_reserveA down = newCandidate(head.r + 1, head.c, cost);
				if (down != null) newStep.add(down);

				Pos_reserveA left = newCandidate(head.r, head.c - 1, cost);
				if (left != null) newStep.add(left);

				Pos_reserveA right = newCandidate(head.r, head.c + 1, cost);
	...
	static void printPath() {
		...
		for (int i=0; i<cases.size(); i++) {
			Pos_reserveA p = cases.get(i).peekFirst();
			if (p != null)
				pathMap[p.r][p.c] = i+1;
		}
	...
	static Pos_reserveA newCandidate(int r, int c, int cost) {
		if (isValid(r, c)) {
...
class Pos_reserveA {
	...
	public boolean equals(Object o) {
		if (o instanceof Pos_reserveA) {
			Pos_reserveA p = (Pos_reserveA) o;
			return (p.r == r && p.c == c);
		}
